...
# The Thorlabs wrapper originally hard‑coded the 64‑bit DLL.  We now
# inspect the Python interpreter's architecture so the correct library
# is loaded (and fall back gracefully if it's not present).
_bits = platform.architecture()[0]
if _bits == "64bit":
    _dll_candidates = ["FilterWheel102_win64.dll", "FilterWheel102_win32.dll"]
else:
    _dll_candidates = ["FilterWheel102_win32.dll", "FilterWheel102_win64.dll"]

FWxCLib = None
_last_exc = None
# make the search path explicit: the DLLs sit in the same folder as
# this Python module, which might not be on the DLL search path when
# the code runs (PyInstaller, import time, etc.).
_sdk_dir = os.path.dirname(__file__)
for _dll in _dll_candidates:
    _fullpath = os.path.join(_sdk_dir, _dll)
    try:
        import sys as _sys
        # stderr may be None when running a GUI build; guarding to avoid
        # throwing during module import.
        try:
            _sys.stderr.write(f"[FWxC] trying to load '{_fullpath}'\n")
        except Exception:
            pass
        FWxCLib = cdll.LoadLibrary(_fullpath)
        try:
            _sys.stderr.write(f"[FWxC] successfully loaded '{_fullpath}'\n")
        except Exception:
            pass
        break
    except OSError as e:
        try:
            _sys.stderr.write(f"[FWxC] could not load '{_fullpath}': {e}\n")
        except Exception:
            pass
        _last_exc = e
if FWxCLib is None:
    # re‑raise the last error to propagate import failure
    raise _last_exc
